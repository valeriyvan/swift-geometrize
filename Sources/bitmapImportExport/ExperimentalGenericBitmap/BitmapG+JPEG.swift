...
public extension BitmapG where ColorType == Rgba8888 {
    ...
    func jpegData(blending background: Rgba8888 = .white) throws -> Data {
        let rgb: [JPEG.RGB] = backing
            // .lazy // TODO: make lazy work!
            .chunks(ofCount: 4)
            .map { Rgba8888($0).blending(background: background) }
    ...
    init(jpegData data: Data) throws {
        ...
        var bitmap = BitmapG<Rgba8888>(width: width, height: height, data: rgba)
        let exifOrientation = BitmapG<Rgba8888>.ExifOrientation(rawValue: image.exifOrientation()) ?? .up
    ...
    init(jpegUrl url: URL) throws {
        ...
        self = BitmapG<Rgba8888>(width: image.size.x, height: image.size.y, data: data)
    }
